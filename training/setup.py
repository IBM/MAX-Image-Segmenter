...
from setup.yaml_handling import YAMLHandlder
...
if len(sys.argv) == 2:
    ...
    try:
        with open(file_name) as fp:
            ...
            assert ("bucket" in config['train']['data_source']['training_data'])
            assert("bucket" in config['train']['model_training_results']['trained_model'])
            assert("path" in config['train']['data_source']['training_data_local'])
            assert("name" in config['train']['execution']['compute_configuration'])
    except:
        print("[DEBUG] Provide a valid configuration YAML file for initiating the training process")
        sys.exit()

def yaml_handle(read_flag, input_bucket_name, local_directory, result_bucket_name, compute_config):
    """
    ...
    :param read_flag: indicator for choosing the operation to be performed.
    :param input_bucket_name: input bucket name
    :param local_directory: local directory from where data needs to be uploaded
    ...
    try:
        with open(file_name) as fp:
            ...
            if read_flag == 'Y':
                cfg_inp_bucket = config['train']['data_source']['training_data']['bucket']
                cfg_out_bucket = config['train']['model_training_results']['trained_model']['bucket']
                cfg_loc_path = config['train']['data_source']['training_data_local']['path']
                cfg_cmp_config = config['train']['execution']['compute_configuration']['name']
                return cfg_inp_bucket, cfg_out_bucket, cfg_loc_path, cfg_cmp_config
            if read_flag == 'N':
                with open(file_name, 'w') as fp:
                    config['train']['data_source']['training_data']['bucket'] = input_bucket_name
                    config['train']['model_training_results']['trained_model']['bucket'] = result_bucket_name
                    config['train']['data_source']['training_data_local']['path'] = local_directory
                    config['train']['execution']['compute_configuration']['name'] = compute_config
                # Updating configuration file with new values
                    yaml.dump(config, fp)
                    return True
            if read_flag == 'C':
                with open(file_name, 'w') as fp:
                    # Updating configuration file with new compute configuration values.
                    config['train']['execution']['compute_configuration']['name'] = compute_config
                    yaml.dump(config, fp)
                    return True
...
def env_extract(access_key, secret_access_key, username, password, instance_id, url):
    """
    This function:
    1. Extract current configuration values from the YAML file.
    2. Handle input and result buckets. Upload data to input bucket if necessary.
    3. Retrieve compute configuration value.
    4. Update YAML file with new values.
    5. Display environment variables value required to be set for initiating training process.
    ...
    The function exits by displaying environment variables value to be set along with configuration variables
    values that have been updated in YAML file.
    """
    cfg_inp_bucket, cfg_out_bucket, cfg_loc_path, cfg_cmp_config = yaml_handle('Y', '', '', '', '')
    yaml_handler = YAMLHandlder(cfg_inp_bucket, cfg_out_bucket, cfg_loc_path, cfg_cmp_config, access_key, secret_access_key)
    ...
    print('------------------------------------------------------------------------------')
    config_result = yaml_handle('N', input_bucket_name, local_directory, result_bucket_name, compute_config)
    if config_result:
        ...
        print('        i) ML_USERNAME={0}                                             '.format(username))
        print('       ii) ML_PASSWORD={}                                              '.format(password))
        print('      iii) ML_INSTANCE={}                                              '.format(instance_id))
        print('       iv) ML_ENV={}                                                   '.format(url))
        print('        v) AWS_ACCESS_KEY_ID={}                                        '.format(access_key))
        print('       vi) AWS_SECRET_ACCESS_KEY={}                                    '.format(secret_access_key))
        print('                                                                                   ')
        print('  2. Run `python train.py {} prepare` to verify your step.             '.format(sys.argv[1]))
        print('  3. Run `python train.py {}` to train the model using your data.      '.format(sys.argv[1]))
        print('                                                                                   ')
        print('************************************************************************************')
    sys.exit()
...
env_check = """
            *------------------------------------------------------------------------------------------------*
            |  Model Asset Exchange - One place for all state of the art Open Source deep learning models.   |
            *------------------------------------------------------------------------------------------------*
            ...
            *       This step is required if IBM Cloud API Key JSON file has not been downloaded before       *
            *                                                                                                 *
            *                                                                                                 *
            *       1. Login to IBM Cloud                                                                     *
            *       2. Go to Manage -> Access(IAM) -> IBM Cloud API Keys                                      *
            *       3. Click `Create an IBM Cloud API Key`. Provide `name` and `description`.                 *
...
if wml_env_check_flag == 'N':
    print(' ')
    print('[MESSAGE] Watson Machine Learning environment variables are not defined.')
else:
    print('[MESSAGE] Watson Machine Learning environment variables are set.')
...
if cos_env_check_flag == 'N':
    print(' ')
    print('[MESSAGE] Cloud Object Storage environment variables are not defined.')
else:
    print('[MESSAGE] Cloud Object Storage environment variables are set.')

# steps to perform if environment variables are set
if cos_env_check_flag == 'Y' and wml_env_check_flag == 'Y':
    selection = """
            *-------------------------------------------------------------------------------------------------*
            |     Environment variables have already been set. Choose an option from below for next steps     |
            |--------------------------------------------------------------------------------------------------
            |    MENU                                                                                         |
            |                                                                                                 |
    ...
    if user_option == '1':
        ...
        env_extract(access_key, secret_access_key, username, password, instance_id, url)
    ...
    if user_option == '3':
        ...
        if config_result:
            print('------------------------------------------------------------------------------')
            print("[MESSAGE] Compute configured has been changed to {}.".format(compute_config))
            print('------------------------------------------------------------------------------')
        sys.exit()
...
resource_id_display = """
                  *------------------------------------------------------------------------------------------*
                  ...
                  |                                                                                          |
                  |   A resource group is a way for you to organize your account resources in customizable   |
                  |   groupings. When you create a service instance for one of the services (e.g. Watson     |
                  |   Machine Learning), it is assigned to a particular resource group. Once assigned to a   |
                  |   resource group, it can't be changed.                                                   |
                  |                                                                                          |
                  *------------------------------------------------------------------------------------------*
                          """
...
config_display = """
                  *-------------------------------------------------------------------------------------------*
                  ...
                  |                                                                                           |
                  |  Instance: When you purchase a service on IBM Cloud, an instance of that service is       |
                  |            provisioned for you to use.                                                    |
                  |                                                                                           |         
                  |  Service Credentials(Key): Service credentials are authentication credentials associated  |
                  |                            with a service. It is used for interacting with a service.     |
                  |                                                                                           |
                  |  To design, train, and deploy machine learning models in IBM Watson Studio, you need to   |
                  |  associate an IBM Watson Machine Learning service instance as well as some supporting     |
                  |  services (such as IBM Cloud Object Storage for storage) with a project.                  |
                  |                                                                                           |
                  *-------------------------------------------------------------------------------------------*
    """
...
if cos_env_check_flag == 'N' or wml_env_check_flag == 'N':
    flow_check_flag = 'N'
    if cos_env_check_flag == 'N' and wml_env_check_flag == 'N' and change_setting_flag == 'N' and flow_check_flag == 'N':
        print('   ')
        print("*-------------------------------------------------------------------------------*")
        print("|  Configuring both Watson Machine Learning and Cloud Object Storage.           |")
        print("*-------------------------------------------------------------------------------*")
        option = 'both'
        flow_check_flag = 'Y'

    if cos_env_check_flag == 'N' and change_setting_flag == 'N' and flow_check_flag == 'N':
        selection = """
    ...
    if wml_env_check_flag == 'N' and change_setting_flag == 'N' and flow_check_flag == 'N':
        selection = """
    ...
    main_handle = MainHandler(resource_id, iam_access_token, ins_obj, ins_handle)
    #
    ...
    if option == 'both':
        ...
        try:
            ...
            resource_instance_id, apikey, access_key, secret_access_key = main_handle.cos_block()
        except KeyError as ex:
            print(type(ex), '::', ex)
            sys.exit()
    ...
    if option == 'wml':
        try:
            ...
            sys.exit()
        print('***** Watson Machine Learning setting has been completed. *****')
        access_key = os.environ['AWS_ACCESS_KEY_ID']
        secret_access_key = os.environ['AWS_SECRET_ACCESS_KEY']
        env_extract(access_key, secret_access_key, username, password, instance_id, url)
        sys.exit()
