...
class InstanceHandler:
    ...
    def available_instance(self, which_resource, resource_id):
        """
        This function retrieves available instances of specified service and prompts user to select the instance
        to be used for training.
        1. Available service resource plan ids of instances are checked against the standard service resource plan id
           values corresponding to the provided service and resource id.
        2. Displays the available resource plan ids of instances under the chosen service.
        3. Prompts user to enter the index of the displayed instances of their choice. User can also choose to create
           a new instance.
        4. If there is no available instance, user will be taken directly to instance creation step.
        :param which_resource: under which service, standard service resource plan id needs to be searched.
        :param resource_id: under which resource, availability of instances need to be searched.
        :return: List of existing instances, guids and selected option. Exit on error
        """
        headers = {
            'Authorization': self.iam_access_token,
        }
        response = requests.get("https://resource-controller.cloud.ibm.com/v2/resource_instances", headers=headers)
        if response.status_code == 200:
            ...
            for index, value in enumerate(response['resources'], start=1):
                ...
                if value['resource_group_id'] == resource_id:
                    for index_1, plan in enumerate(value['plan_history'], start=1):
                        for key, values in enumerate(self.data[which_resource].items()):
                            if plan['resource_plan_id'] == values[1]:
                                count += 1
                                print("{}. Instance Name: {}   |  Instance Location: {}  | Instance Plan: {} ".
                                      format(int(count), value['name'], value['region_id'], values[0]))
                                existing_instances.append(value['name'])
                                existing_guids.append(value['guid'])
            # Adding create new instance option.
            if len(existing_instances) > 0:
                print('{}. {}'.format(int(count) + 1, '* Create New Instance *'))
            ...
            if len(existing_instances) > 0 and existing_instances[0] != 'Create New Instance':
                while True:
                    instance_option = input("[PROMPT] Your selection:  ")
                    if instance_option == '':
                        print("[MESSAGE] Enter number between 1 and {}.".format(len(existing_instances)))
                        continue
                    elif not instance_option.isdigit():
                        print("[MESSAGE] Enter number between 1 and {}.".format(len(existing_instances)))
                        continue
                    elif int(instance_option) < 1 or (int(instance_option) >= (len(existing_instances) + 1)):
                        print("[MESSAGE] Enter number between 1 and {}.".format(len(existing_instances)))
                        continue
                    else:
    ...
    def wml_key_check(self, instance_guid):
        """
        This function:
        1. Retrieves list of keys under provided instance guid.
        2. Prompts user choice to choose from existing keys or create new key
        :param instance_guid: guid of instance under which presence of keys need to be searched.
        :return: list of existing keys, guid of keys and user choice. Exit when error occurs in key retrieval request.
        """
        headers = {
            'Authorization': self.iam_access_token,
        }
        available_keys = requests.get("https://resource-controller.cloud.ibm.com/v2/resource_keys", headers=headers)
        if available_keys.status_code == 200:
            ...
            if len(existing_keys) > 0:
                while True:
                    ...
                    if key_option == '':
                        print("[MESSAGE] Enter number between 1 and {}.".format(len(existing_keys)))
                        continue
                    elif not key_option.isdigit():
                        print("[MESSAGE] Enter number between 1 and {}.".format(len(existing_keys)))
                        continue
                    elif int(key_option) < 1 or int(key_option) >= (len(existing_keys) + 1):
                        print("[MESSAGE] Enter number between 1 and {}.".format(len(existing_keys)))
                        continue
                    else:
            ...
            print("[DEBUG] Failing with status code:", available_keys.status_code)
            print("[DEBUG] Reason for failure:", available_keys.reason)
            sys.exit()

    def cos_key_check(self, instance_guid):
        """
        This function:
        1. Retrieves list of keys under provided instance guid.
        2. Prompts user choice to choose from existing keys or create new key
            ...
            if len(existing_keys) > 0:
                while True:
                    ...
                    if key_option == '':
                        ...
                        return existing_keys, key_option, existing_key_guid
            else:
                existing_keys = ['Create New Key']
                key_option = '1'
                existing_key_guid = []
