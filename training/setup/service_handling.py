...
class ServiceHandler:
    ...
    def get_resources_id(self):
            """
            Retrieve available resources. Prompt user choice if there are more than one resource.
            :return: resource id. Exit on error in resource id retrieval.
            """
            headers = {
                'Authorization': self.iam_access_token,
            }
            response = requests.get('https://resource-controller.cloud.ibm.com/v2/resource_groups', headers=headers)
            if response.status_code == 200:
                ...
                if len(resources_list) == 1:
                    print(' ')
                    print("[MESSAGE] Using the default available resource '{}'.".format(resources_name[0]))
                    return resources_list[0]
                else:
                    while True:
                        resources_id = input("[PROMPT] Your selection:  ")
                        if resources_id == '':
                            print("[MESSAGE] Enter a number between 1 and {}.".format(len(resources_name)))
                            continue
                        elif not resources_id.isdigit():
                            print("[MESSAGE] Enter a number between 1 and {}.".format(len(resources_name)))
                            continue
                        elif int(resources_id) < 1 or int(resources_id) >= (len(resources_list) + 1):
                            print("[MESSAGE] Enter a number between 1 and {}.".format(len(resources_name)))
                            continue
                        else:
                            break
                    print('[MESSAGE] Selected resource is option {}'.format(resources_id))
    ...
    def get_wml_deployment_details(self):
        """
        Get Watson Machine Learning deployment location, plan and resource plan id
        ...
        print("[MESSAGE] Choose Watson Machine Learning deployment location from the displayed options. ")
        ...
        while True:
            ...
            if wml_dep_option == '':
                ...
                print('-----------------------------------------------------------------------------------')
                print('[MESSAGE] Chosen Watson Machine Learning location is : {}'.
                      format(wml_deployment_location[int(wml_dep_option) - 1]))
                print('-----------------------------------------------------------------------------------')
                break
        print("[MESSAGE] Choose Watson Machine Learning (WML) plan from the displayed options. ")
        ...
        return wml_deployment_location[int(wml_dep_option) - 1], wml_plans[int(wml_plan_option) - 1], \
    ...
    def get_cos_deployment_details(self):
        ...
        print("[MESSAGE] Choose Cloud Object Storage plan from the displayed options. ")
        ...
        while True:
            ...
            if cos_plan_option == '':
                ...
                print('-----------------------------------------------------------------------------------')
                print('[MESSAGE] Chosen Cloud Object Storage plan is : {}'.format(cos_plans[int(cos_plan_option) - 1]))
                print('-----------------------------------------------------------------------------------')
                break
        # Read config file to extract resource plan id
        with open('setup/config.json') as config_file:
            data = json.load(config_file)
        return cos_plans[int(cos_plan_option) - 1], data["cos"][cos_plans[int(cos_plan_option) - 1]]

    def service_create(self, instance_name, location, resource_id, resource_plan_id):
        """
        ...
        :param resource_id: Resource id under which instance needs to be created
        ...
        response = requests.post("https://resource-controller.cloud.ibm.com/v2/resource_instances", headers=headers,
                                     json=data_instance)
        # Response error check
        if response.status_code == 201:
            response = response.json()
            print("[MESSAGE] Instance with the name '%s' has been created" % response['name'])
    ...
    def wml_key_create(self, wml_key_name, wml_instance_guid):
        ...
        :return: WML username, password, key instance_id and password. Exit on error in key creation.
        """
        headers = {
            'Authorization': self.iam_access_token,
        }
        ...
        # Instance create
        wml_key_response = requests.post('https://resource-controller.cloud.ibm.com/v2/resource_keys', headers=headers,
                                         json=data_wml)
        # Response error check
        if wml_key_response.status_code == 201:
            wml_key_response = wml_key_response.json()
            print("[MESSAGE] WML key with the name '%s' has been created" % wml_key_response['name'])
            return wml_key_response['credentials']['username'], wml_key_response['credentials']['password'], \
                    wml_key_response['credentials']['instance_id'], wml_key_response['credentials']['url']
        else:
            print("[DEBUG] Failing with status code:", wml_key_response.status_code)
            print("[DEBUG] Reason for failure:", wml_key_response.reason)
            sys.exit()
    ...
    def cos_key_create(self, cos_key_name, cos_instance_guid):
        ...
        :return: instance id, apikey, access key id and secret access key. Exit on error in key creation.
        ...
        # Key creation
        cos_key_response = requests.post('https://resource-controller.cloud.ibm.com/v2/resource_keys', headers=headers,
                                         json=data_obj)
        # Error check
        if cos_key_response.status_code == 201:
            cos_key_response = cos_key_response.json()
            print("[MESSAGE] Object storage key with the name '%s' has been created" % cos_key_response['name'])
            return cos_key_response['credentials']['resource_instance_id'], cos_key_response['credentials']['apikey'], \
                   cos_key_response['credentials']['cos_hmac_keys']['access_key_id'], \
                   cos_key_response['credentials']['cos_hmac_keys']['secret_access_key']
        else:
            print("[DEBUG] Failing with status code:", cos_key_response.status_code)
            print("[DEBUG] Reason for failure:", cos_key_response.reason)
            sys.exit()
